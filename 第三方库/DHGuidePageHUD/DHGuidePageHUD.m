...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        // 设置引导页上的跳过按钮
        UIButton *skipButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.8, DDScreenW*0.1 + IS_iPhoneX_Top, 50, 25)];
        [skipButton setTitle:NSLocalizedString(@"jumpButton", nil) forState:UIControlStateNormal];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.3, DDScreenH*0.85, DDScreenW*0.4, 44)];
                [startButton setTitle:NSLocalizedString(@"StartExperiencingButton", nil) forState:UIControlStateNormal];
                [startButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                [startButton.titleLabel setFont:[UIFont systemFontOfSize:16]];
                [startButton setBackgroundImage:[UIColor bm_colorGradientChangeWithSize:startButton.bounds.size direction:GradientChangeDirectionLevel startColor:[UIColor colorWithHex:0xff59bbff] endColor:[UIColor colorWithHex:0xff4a88ff]] forState:UIControlStateNormal];
        ...
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithHex:0xff4a88ff];
