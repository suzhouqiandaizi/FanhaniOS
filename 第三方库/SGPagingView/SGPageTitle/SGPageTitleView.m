...
#define IndicatorViewWidth 30.0f
...
/// 按钮之间的间距
static CGFloat const SGPageTitleViewBtnMargin = 50;
...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames {
    if (self = [super initWithFrame:frame]) {
        ...
        self.titleArr = [NSMutableArray arrayWithArray:titleNames];
...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.backgroundColor = ThemeColor;
    }
...
- (UIView *)bottomSeparator {
    if (!_bottomSeparator) {
        ...
        _bottomSeparator.backgroundColor = LineColor;
    }
...
- (void)changeIndicatorViewLocationWithButton:(UIButton *)button {
    if (self.allBtnWidth <= self.bounds.size.width) { /// SGPageTitleView 不可滚动
        [UIView animateWithDuration:_indicatorAnimationTime animations:^{
            if (self.indicatorLengthStyle == SGIndicatorLengthTypeCustom) {
                self.indicatorView.SG_width = IndicatorViewWidth;
            }else if (self.indicatorLengthStyle == SGIndicatorLengthTypeEqual) {
                ...
                self.indicatorView.SG_width = button.SG_width;//改变底部指示器宽度
            }
...
- (void)setPageEndDeceleratingwithTargetIndex:(NSInteger)targetIndex;{
    UIButton *button = [self.btnMArr objectAtIndex:targetIndex];
    [UIView animateWithDuration:_indicatorAnimationTime animations:^{
        if (self.indicatorLengthStyle == SGIndicatorLengthTypeCustom) {
            self.indicatorView.SG_width = IndicatorViewWidth;
        }else if (self.indicatorLengthStyle == SGIndicatorLengthTypeEqual) {
...
- (void)isIndicatorScrollWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    if (self.indicatorLengthStyle == SGIndicatorLengthTypeEqual) {
        ...
        distance = progress * (totalDistance - totalOffsetX);
        /// 计算 indicatorView 新的 frame
        self.indicatorView.SG_x = originalBtn.SG_origin.x + offsetX - ((IndicatorViewWidth- originalBtn.SG_width)/2.0);
        self.indicatorView.SG_width = IndicatorViewWidth;
    }else {
        offsetX = totalOffsetX * progress;
        distance = progress * (totalDistance - totalOffsetX);
...
- (void)setIndicatorLengthStyle:(SGIndicatorLengthType)indicatorLengthStyle {
    ...
    if (self.indicatorLengthStyle == SGIndicatorLengthTypeCustom) {
        indicatorViewW = IndicatorViewWidth;
        indicatorViewX = firstBtn.SG_x;
    }else if (indicatorLengthStyle == SGIndicatorLengthTypeEqual) {
...
- (void)resetTitleWithIndex:(NSInteger)index newTitle:(NSString *)title {
    if (index < self.btnMArr.count) {
        ...
        [self.titleArr replaceObjectAtIndex:index withObject:title];
